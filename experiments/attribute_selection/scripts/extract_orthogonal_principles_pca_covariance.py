...
def main(cfg: DictConfig) -> None:
    ...
    # Center only (no variance normalization) — PCA on covariance matrix weights by natural variance
    scores_std = (scores_matrix - scores_matrix.mean(axis=0)) / (scores_matrix.std(axis=0) + 1e-8)
    corr_matrix = np.corrcoef(scores_std, rowvar=False)  # kept for antonym check only
    ...
    # Parallel analysis to choose k — random baseline must also use covariance
    n_iterations = 100
    random_eigenvalues = np.zeros((n_iterations, num_principles))
    raw_std = scores_matrix.std(axis=0)
    for i in range(n_iterations):
        random_data = np.random.normal(size=(num_conversations, num_principles)) * raw_std
        random_cov = np.cov(random_data.T)
        random_eigenvalues[i] = np.sort(np.linalg.eigh(random_cov)[0])[::-1]
    ...
    for budget in [4, 8, 16, k, 32]:
        ...
        json_path = npy_path.with_name(npy_path.stem + f'_groups_pca_cov_k{budget}.json')
        out['method'] = 'pca_cov'
    ...
    plt.title(f'Scree Plot with Parallel Analysis — covariance (k={k})')
    plt.axvline(x=k, color='gray', linestyle=':', alpha=0.5, label=f'k={k}')
    plt.legend()
    plt.tight_layout()
    plt.savefig(npy_path.with_name(npy_path.stem + '_scree_cov.png'), dpi=150)
    print(f"Saved {npy_path.with_name(npy_path.stem + '_scree_cov.png')}")

    # Save
    np.savez(
        npy_path.with_name(npy_path.stem + '_pca_cov_results.npz'),
    ...
    print(f"Saved {npy_path.with_name(npy_path.stem + '_pca_cov_results.npz')}")
